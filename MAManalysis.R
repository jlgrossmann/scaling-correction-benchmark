# this function (MAManalysis) is modified from the SensMixed package (license: GPL-2)
# original authors:
...
# the function was modified to accept only a single sensory attribute at a time
# the function is only suitable for balanced data. the SensMixed package 
# also offers a function for unbalanced data. As SensMixed is currently unavailable
# on CRAN, the easiest way to install it should be 
# install.packages("SensMixed", repo="https://cran.microsoft.com/snapshot/2020-02-17/") [not tested] or
# remotes::install_url('https://cran.r-project.org/src/contrib/Archive/SensMixed/SensMixed_2.1-0.tar.gz') [not tested]
# requires package doBy


MAManalysis = function(data, adjustedMAM=T, alpha_conditionalMAM=0.2){
  # data:
  ...
  # There MUST be at least 1 attribute in the data frame "data"
  ...
  # If p-value of the scaling differences is greater than alpha_conditionalMAM, i.e. the
  # scaling is non-significant, a traditional 2-way mixed ANOVA composition is performed
  # and the results are identical to using aov(). 
  ...
  data=doBy::orderBy(~ass+prod,data)
  ...
  mins=ifelse(ncol>4, apply(data[,4:ncol],2,min), min(data[,4]))
  maxs=ifelse(ncol>4, apply(data[,4:ncol],2,max), max(data[,4]))
  ranges=maxs-mins
  ...
  for (p in 4:ncol){ # running through attributes one by one
    ...
    mu=mean(X)
    data$X=X
    xam=doBy::summaryBy(X~ass,data=data)  # assessor means
    xpm=doBy::summaryBy(X~prod,data=data)  # product means
    ...
    xapm=doBy::summaryBy(X~ass+prod,data=data)  # assessor:product means
    xapm$int=xapm$X.mean-rep(xam$X.mean,rep(nprod,nass))-rep(xpm$X.mean,nass)+mu  # repeat 
    ssa=nprod*nrep*(xam$X.mean-mu)^2  # assessor SS
    ...
    sses=X-ave(X,factor((data$ass):(data$prod)))  # error
    
    # ss for each assessor
    for (i in 1:nass){
      ssp[i]=anova(lm(X~prod,data=subset(data,ass==i)))[1,2]  # product SS from lm
    ...
    # sums of squares of effects
    # 1 value per assessor
    indivTable[1,,p-3]=ssa     # assessor
    indivTable[2,,p-3]=ssp     # product
    indivTable[3,,p-3]=sssca   # scaling
    indivTable[4,,p-3]=ssdis   # disagreement
    indivTable[5,,p-3]=sse     # error
    indivTable[6,,p-3]=beta+1  # scaling coefficient
    indivTable[7,,p-3]=scale(assmeans[,p-3],scale=F)  # assessor effect
  ...
  # sum indivTable across assessors (sums of squares)
  #  to generate SS
  ...
  # mean squares
  msa=aovs[1,]/(nass-1)  # assessor
  msp=aovs[2,]/(nprod-1)  # product
  msc=aovs[3,]/(nass-1)  # scaling
  msd=aovs[4,]/((nass-1)*(nprod-2))  # disagreement
  mse=aovs[5,]/(nass*nprod*(nrep-1))  # error
  msint=(aovs[3,]+aovs[4,])/((nprod-1)*(nass-1))  # interaction (disagreement + scaling)
  
  # calculate assessor statistics
  for (p in 4:ncol){ # running through attributes one by one
    perfTable[1,,p-3]=nass*indivTable[1,,p-3]/(msint[p-3]*(nass-1))  # assessor/interaction
    perfTable[2,,p-3]=1-pf(perfTable[1,,p-3],1,(nass-1)*(nprod-1))  # assessor p's
    ...
    perfTable[5,,p-3]=1-pf(perfTable[4,,p-3],nprod-1,nprod*(nrep-1))  # product p's
    perfTable[6,,p-3]=indivTable[3,,p-3]/(indivTable[4,,p-3]/(nprod-2))
    perfTable[7,,p-3]=1-pf(perfTable[6,,p-3],1,(nprod-2))  # scaling p
    perfTable[8,,p-3]=(indivTable[6,,p-3]^2)*(aovs[2,p-3]/(nass))/(indivTable[4,,p-3]/(nprod-2))
    perfTable[9,,p-3]=1-pf(perfTable[8,,p-3],1,(nprod-2))  # disagreement p
    ...
    IPtabT=matrix(as.character(round(IPTable[,,p-3],3)),nrow=6)
    PIPtabT=IPtabT
    for (i in 1:6){
      PIPtabT[i,1:nass]=" "
  ...
  # adjusted MAM: move variance from scaling to disagreement
  if (adjustedMAM) {
    negatives=(indivTable[6,,]<0)
    if (natt==1) negatives = matrix(negatives)
    nnegatives=apply(negatives,2,sum)
    for (p in 1:natt){
      aovs[3,p]=aovs[3,p]-sum(indivTable[3,,p][negatives[,p]])  # scaling effect (becomes smaller)
      aovs[4,p]=aovs[4,p]+sum(indivTable[3,,p][negatives[,p]])  # disagreement effect (becomes larger)
  ...
  # calculate F values
  Fmatr[1,]=msa/msint  # assessor effect (over interaction, the ANOVA way)
  Fmatr[2,]=msp/msd  # product effect (over disagreement) (if p(scaling)<alpha_conditionalMAM)
  Fmatr[3,]=msc/msd  # scaling
  Fmatr[4,]=msd/mse  # disagreement
  
  Fmatr[5,]=msp/msint  # product effect over interaction (not MAM, if p(scaling)>alpha_conditionalMAM)
  Fmatr[6,]=msint/mse  # interaction / error
  Fmatr[7,]=((aovs[2,]+aovs[3,])/(nprod+nass-2))/msd  # scaling+product / disagreement
  
 
  # calculate p values
  # DF_num  ass   prod    scaling     disagreement   prod/int      int/err           
  DFnums=c(nass-1,nprod-1,nass-1,(nprod-2)*(nass-1),nprod-1,(nprod-1)*(nass-1),(nprod+nass-2))
  #             interaction     disagreement      disagreement      error          
  DFdens=c((nprod-1)*(nass-1),(nprod-2)*(nass-1),(nprod-2)*(nass-1),nass*nprod*(nrep-1),
           (nprod-1)*(nass-1),nass*nprod*(nrep-1),(nprod-2)*(nass-1))
  
  for (i in 1:7) Pmatr[i,]=1-pf(Fmatr[i,],DFnums[i],DFdens[i])  # calculate p-values of effects
  

  # AOVtable columns: SS, MS, DF, F, Pval; rows: assessor, product, scaling, disagreement, error
  AOVtable[,1,]=round(aovs,4)  # SS
  
  if (adjustedMAM) {  # adjust for negative scalings
    for (p in 1:natt) {
              # 1.ass  2.prod      3.scaling               4.disagreement
      DFnums=c(nass-1, nprod-1, nass-1-nnegatives[p], (nprod-2)*(nass-1)+nnegatives[p], 
               nprod-1, (nprod-1)*(nass-1), (nprod+nass-2-nnegatives[p]))
      AOVtable[,3,p]=c(DFnums[1:4], nass*nprod*(nrep-1))  # DFs + DF(error)
      ...
      AOVtable[,2,p]=round(aovs[,p]/AOVtable[,3,p],4)  # MS = SS/DF
    }
  }
  
  if (!adjustedMAM) {  # else: calculate dfs and MS "traditionally" (the MAM way, i.e. (I-1)(J-2))
    dfs=c(nass-1,nprod-1,nass-1,(nprod-2)*(nass-1),nass*nprod*(nrep-1))
    AOVtable[,3,]=dfs
    AOVtable[,2,]=round(aovs/dfs,4)
  }
  
  # round
  AOVtable[1:4,4,]=round(Fmatr[1:4,],6)  # F values
  AOVtable[1:4,5,]=signif(Pmatr[1:4,],6)  # p values
  
  
  # Here the MAM condition is applied, such that the product F may become the usual one again 
  # if no scaling difference is detected (MAMcondition=F, p>0.2), the MAM is not applied, 
  #  i.e. the F-value is calculated over the MS(interaction) 
  # if a scaling difference is detected (MAMcondition=T, p<0.2), the MAM is applied. 
  MAMcondition=(Pmatr[3,]<=alpha_conditionalMAM)  # check if scaling is significant for each attribute
  scaling_significant = MAMcondition
  AOVtable[2,4,][!MAMcondition]=round(Fmatr[5,][!MAMcondition],4)  # for those attributes that have no significant scaling, replace Fv=MS(v)/MS(d) with Fv=MS(v)/MS(int) 
  AOVtable[2,5,][!MAMcondition]=signif(Pmatr[5,][!MAMcondition],4)  # for those attributes that have no significant scaling, replace pv [MS(v)/MS(d)] with pv [MS(v)/MS(int)] 
  ...
  IndividualTable = array(dim=c(5,nass,natt))
  for (i in 1:natt) IndividualTable[,,i] = indivTable[1:5,,i]
  ...
  # The potential "unadjustment" (undoing the adjustment):
  if (adjustedMAM) {
    negatives = matrix(indivTable[6,,]<0)
    if (natt==1) negatives = matrix(negatives)
    for (p in 1:natt){
  ...
  list(IndividualTable, FinalTable, aov=AOVtable, DIFtable, lsmeans.tab, ProdTable,
       CLTable, ci.tab, 
       adj.MAM.negatives=nnegatives)
}
