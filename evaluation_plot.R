sim_results = readRDS('out_permutation_all_cheese-Fattiness_2022-11-29_18.01.37.Rds')


if(sim_results$parameters$perm){
  # calculate p-values from permutation
  # with less than 10 exceedances, the p-value becomes less accurate, signified by a (!)
  ...
  sim_results$f_table = sim_results$f_table[-1,]
  pvals
}


# Figure 1
histobreaks = 0:150/15
...
matplot(f_density$raw$mids, sapply(f_density, function(x) x$density)[,1:6], type='l', lty=1, xlab=expression(F[nu]), ylab='density', 
        main=expression(F[nu]~"null distributions (unrestricted perm.)"), xlim=c(0.15,4.85), ylim=c(0.02,1.1), col=0)
...
matplot(histobreaks2[-1]-histobreaks2[2]/2, p_density[,1:6], type='l', lty=1, xlab=expression(p[nu]), ylab='density', 
        main=expression(p[nu]~"distributions (unrestricted perm.)"), xlim=c(0.02,0.98), ylim=c(0,3), col=0)
